...
pumshh <- get_pums(year = 2019,
                 state = c("TX", "CA"), 
                 survey = "acs1",
                 variables =c("ST", "NP", "ACCESS", "HISPEED", "TEN", "VALP"),
                 recode = T,
...
readr::write_csv(pumshh, file  = "~/Documents/Github/data/pums_tx_ca_hh_2019.csv")


write.csv(vars[vars$var_code%in%names(pums), ], file = "~/Documents/Github/data/pums_vars.csv", row.names = F)
write.csv(vars[vars$var_code%in%names(pumshh), ], file = "~/Documents/Github/data/pums_hh_vars.csv", row.names = F)
